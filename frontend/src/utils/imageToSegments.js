...
function workerFn() {
  self.onmessage = function(e) {
    ...
    const segs    = traceContours(
      edges, rgba, w, h,
      color, lineWidth, instrument,
      !color, parsedPalette,
      minSegmentLen, maxSegments, simplifyEps
    );
    ...
    for (let y = 1; y < h-1; y++) {
      ...
      for (let x = 1; x < w-1; x++) {
        ...
        if (usePaletteMatching) {
          const entry = closestPaletteEntry(sumR/count, sumG/count, sumB/count, palette);
          segColor      = entry.color;
          segInstrument = entry.instrument;
        }

        segments.push({ points, color: segColor, lineWidth, isErase: false, instrument: segInstrument });
      }
    }
