...
const ACCOMP_VOLUME_RATIO = 0.40;      // громкость аккомпанирующего инструмента относительно ведущего
...
export function assignRoles(processedSegs, T) {
  ...
  // ── Определяем бас ─────────────────────────────────────────────────────────
  const sortedByY = Array.from(byInstrument.entries()).sort((a, b) => b[1].avgY - a[1].avgY);
  let bassInstr = null;
  ...
  const nonBassEntries = Array.from(byInstrument.entries()).filter(([i]) => i !== bassInstr);
...
function checkSpatialSplit(nonBassEntries) {
  ...
  const sorted     = [...nonBassEntries].sort((a, b) => a[1].massCenterX - b[1].massCenterX);
  const left       = sorted[0];
  const right      = sorted[sorted.length - 1];
  const leftMass   = left[1].massCenterX;
  const rightMass  = right[1].massCenterX;

  if (rightMass - leftMass < 0.20) return null;

  const maxPts = Math.max(...nonBassEntries.map(([, d]) => d.totalPoints));
  if (!nonBassEntries.every(([, d]) => d.totalPoints >= maxPts * 0.10)) return null;
  ...
  for (let b = 1; b < BUCKETS - 1; b++) {
    ...
    if (diff > bestDiff) { bestDiff = diff; switchBucket = b; }
  }

  const midX = (switchBucket + 0.5) / BUCKETS;
  return { midX, leftInstr: left[0], rightInstr: right[0], allEntries: nonBassEntries };
}
    ...
    for (let k = 0; k < T; k++) {
      ...
      if (instrName === leftInstr) {
        ...
        if (taktX > fadeEnd) {
          ...
          taktVolMult[k] = ACCOMP_VOLUME_RATIO + (1.0 - ACCOMP_VOLUME_RATIO) * t;
        } else {
...
function applyLengthBasedRoles(nonBassEntries, byInstrument, T) {
  const sorted      = [...nonBassEntries].sort((a, b) => b[1].totalLength - a[1].totalLength);
  const melodyInstr = sorted[0][0];
  const maxLength   = sorted[0][1].totalLength;

  for (const [instrName, data] of nonBassEntries) {
    const isMelody = instrName === melodyInstr ||
